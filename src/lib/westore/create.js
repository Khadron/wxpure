...
function rewritePureUpdate(ctx) {
  ctx.update = function(patch) {
    ...
    return new Promise(resolve => {
      ...
      const diffResult = diff(store.data, store.originData)
      const array = []
      if (Object.keys(diffResult).length > 0) {
        ...
        for (const key in diffResult) {
          updateByPath(
            store.originData,
            key,
            typeof diffResult[key] === 'object' ? JSON.parse(JSON.stringify(diffResult[key])) : diffResult[key]
          )
        }
      }
...
function update(patch) {
  return new Promise(resolve => {
    ...
    const array = []
    if (Object.keys(diffResult).length > 0) {
      for (const key in globalStore.instances) {
        globalStore.instances[key].forEach(ins => {
          if (updateAll || globalStore.updateAll || ins._updatePath) {
            ...
            if (needUpdatePathList.length) {
              const _diffResult = {}
              for (const _path in diffResult) {
                if (needUpdatePathList.includes(_path)) {
                  _diffResult[_path] =
                    typeof diffResult[_path] === 'object'
                      ? JSON.parse(JSON.stringify(diffResult[_path]))
                      : diffResult[_path]
                }
              }
